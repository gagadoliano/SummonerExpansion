...
public class EmpoweredNewEmerald extends TerrainSplatterTile
{
    private final GameRandom drawRandom;

    public EmpoweredNewEmerald()
    {
        super(false, "empowerednewemerald");
        mapColor = new Color(72, 103, 46);
        canBeMined = true;
    ...
    public void tick(Mob mob, Level level, int x, int y)
    {
        if (mob.isPlayer)
        {
            mob.buffManager.addBuff(new ActiveBuff(BuffRegistry.getBuff("emeraldfloorbuff"), mob, 0.5F, mob), true);
        }
    }
    ...
    public ListGameTooltips getItemTooltips(InventoryItem item, PlayerMob perspective)
    {
        ...
        tooltips.add(Localization.translate("itemtooltip", "empoweredemeraldtip"));
        return tooltips;
    }
}