...
public class EmpoweredAmethyst extends TerrainSplatterTile
{
    private final GameRandom drawRandom;

    public EmpoweredAmethyst()
    {
        super(false, "empoweredamethyst");
        mapColor = new Color(74, 61, 99);
        canBeMined = true;
    ...
    public void tick(Mob mob, Level level, int x, int y)
    {
        if (mob.isPlayer)
        {
            mob.buffManager.addBuff(new ActiveBuff(BuffRegistry.getBuff("amethystfloorbuff"), mob, 0.5F, mob), true);
        }
    }
    ...
    public ListGameTooltips getItemTooltips(InventoryItem item, PlayerMob perspective)
    {
        ...
        tooltips.add(Localization.translate("itemtooltip", "empoweredamethysttip"));
        return tooltips;
    }
}