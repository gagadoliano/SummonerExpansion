...
public class MummyMagicDebuff extends Buff
{
    public MummyMagicDebuff()
    {
        canCancel = false;
        isImportant = true;
    }
    ...
    public void clientTick(ActiveBuff buff)
    {
        ...
        if (buff.owner.isVisible())
        {
            ...
            owner.getLevel().entityManager.addParticle(owner.x + GameMath.sin(currentAngle.get()) * distance, owner.y + GameMath.cos(currentAngle.get()) * distance * 0.75F, Particle.GType.CRITICAL).color(new Color(117, 59, 9)).height(0.0F).moves((pos, delta, lifeTime, timeAlive, lifePercent) -> {
