...
import static summonerexpansion.codes.registries.RegistryMobTextures.horrorSpiritBoss;
...
public class HorrorSpiritBossMob extends FlyingHostileMob
{
    ...
    protected void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective)
    {
        ...
        DrawOptions body = horrorSpiritBoss.initDraw().sprite(frame, 0, 108).size(108, 108).light(light).mirror(dir == 0, false).pos(drawX, drawY);
        topList.add((tm) -> {
            body.draw();
        });
    }
