...
import static summonerexpansion.codes.registries.RegistryMinionTextures.bearMinion;

public class BearMinion extends SummonWalkBase
{
    public BearMinion()
    {
        super();
    ...
    public void handleCollisionHit(Mob target, GameDamage damage, int knockback)
    {
        ...
        if (owner != null && target != null)
        {
            ...
            if (getAttackOwner().buffManager.hasBuff("leathersummonersetbonus"))
            {
                buff = new ActiveBuff(BEARBLEEDING, target, 20F, this);
            }
            else
            {
                buff = new ActiveBuff(BEARBLEEDING, target, 10F, this);
            }
    ...
    public void spawnDeathParticles(float knockbackX, float knockbackY)
    {
        for(int i = 0; i < 4; ++i)
        {
            this.getLevel().entityManager.addParticle(new FleshParticle(this.getLevel(), bearMinion, i, 16, 32, this.x, this.y, 20.0F, knockbackX, knockbackY), Particle.GType.IMPORTANT_COSMETIC);
        }
    }
    ...
    public void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective)
    {
        ...
        final DrawOptions options = bearMinion.initDraw().sprite(sprite.x, sprite.y, 128).addMaskShader(swimMask).light(light).pos(drawX, drawY);
