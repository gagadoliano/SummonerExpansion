...
import static summonerexpansion.codes.registries.RegistryDebuffs.WeaponDebuffs.POLARBEARSLOW;
import static summonerexpansion.codes.registries.RegistryMinionTextures.bearPolarMinion;

public class BearPolarMinion extends SummonWalkBase
{
    ...
    public void handleCollisionHit(Mob target, GameDamage damage, int knockback)
    {
        ...
        if (owner != null && target != null)
        {
            if (getAttackOwner().buffManager.hasBuff("frostcrownsetbonus"))
            {
                ActiveBuff buff = new ActiveBuff(POLARBEARSLOW, target, 15F, this);
                target.addBuff(buff, true);
            }
            else
            {
                ActiveBuff buff = new ActiveBuff(POLARBEARSLOW, target, 5F, this);
                target.addBuff(buff, true);
            }
        }
    }
    ...
    public void spawnDeathParticles(float knockbackX, float knockbackY)
    {
        for(int i = 0; i < 4; ++i)
        {
            this.getLevel().entityManager.addParticle(new FleshParticle(this.getLevel(), bearPolarMinion, i, 16, 32, this.x, this.y, 20.0F, knockbackX, knockbackY), Particle.GType.IMPORTANT_COSMETIC);
        }
    }
    ...
    public void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective)
    {
        ...
        final DrawOptions options = bearPolarMinion.initDraw().sprite(sprite.x, sprite.y, 128).addMaskShader(swimMask).light(light).pos(drawX, drawY);
