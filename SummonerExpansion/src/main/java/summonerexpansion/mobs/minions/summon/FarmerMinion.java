...
public class FarmerMinion extends SummonHumanBase
{
    public FarmerMinion()
    {
        super();
        ...
        ai = new BehaviourTreeAI<>(this, new PlayerFollowerCollisionChaserAI<FarmerMinion>(500, summonDamage, 50, 800, 900, 60)
        {
            public boolean attackTarget(FarmerMinion mob, Mob target)
            {
                if (canAttack())
                {
                    ...
                    getLevel().entityManager.events.add(new ToolItemMobAbilityEvent(FarmerMinion.this, GameRandom.globalRandom.nextInt(), attackItem, mob.getX(), mob.getY(), attackAnimTime, attackAnimTime));
