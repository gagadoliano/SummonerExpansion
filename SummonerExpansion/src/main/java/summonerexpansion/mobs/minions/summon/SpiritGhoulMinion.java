...
import static summonerexpansion.codes.registries.RegistryMinionTextures.spiritGhoulMinion;

public class SpiritGhoulMinion extends SummonWalkBase
{
        ...
        ai = new BehaviourTreeAI<>(this, new PlayerFollowerCollisionChaserAI<SpiritGhoulMinion>(600, summonDamage, 50, 900, 800, 100)
        {
            public boolean attackTarget(SpiritGhoulMinion mob, Mob target)
            {
                if (mob.canAttack())
                {
                    ...
                    ActiveBuff ab = new ActiveBuff(dryadHaunted, target, 30F, getAttackOwner());
    ...
    public void spawnDeathParticles(float knockbackX, float knockbackY)
    {
        for(int i = 0; i < 4; ++i)
        {
            getLevel().entityManager.addParticle(new FleshParticle(getLevel(), spiritGhoulMinion, i, 8, 32, x, y, 20.0F, knockbackX, knockbackY), Particle.GType.IMPORTANT_COSMETIC);
        }
    }
    ...
    public void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective)
    {
        ...
        final DrawOptions body = spiritGhoulMinion.initDraw().sprite(sprite.x, sprite.y, 64).addMaskShader(swimMask).light(light).pos(drawX, drawY);
    ...
    public Stream getDefaultModifiers()
    {
