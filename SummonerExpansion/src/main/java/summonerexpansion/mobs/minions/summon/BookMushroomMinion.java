...
import static summonerexpansion.codes.registries.RegistryDebuffs.WeaponDebuffs.MUSHROOMSENTRYSLOW;
import static summonerexpansion.codes.registries.RegistryMinionTextures.bookMushroomMinion;
...
public class BookMushroomMinion extends AttackingFollowingMob
{
    ...
    public void handleCollisionHit(Mob target, GameDamage damage, int knockback)
    {
        ...
        {
            ActiveBuff buff = new ActiveBuff(MUSHROOMSENTRYSLOW, target, 60F, this);
    ...
    public void spawnDeathParticles(float knockbackX, float knockbackY)
    {
        for(int i = 0; i < 6; ++i)
        {
            this.getLevel().entityManager.addParticle(new FleshParticle(this.getLevel(), bookMushroomMinion, i, 8, 32, this.x, this.y, 20.0F, knockbackX, knockbackY), Particle.GType.IMPORTANT_COSMETIC);
        }
    }
    ...
    public void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective)
    {
        ...
        final DrawOptions options = bookMushroomMinion.initDraw().sprite(sprite.x, sprite.y, 64).addMaskShader(swimMask).light(light).pos(drawX, drawY);
