...
import static summonerexpansion.codes.registries.RegistryMinionTextures.woodAxeMinion;

public class WoodAxeMinion extends MagicToolBase
{
    private int toolHit;

    public WoodAxeMinion()
    {
        super();
    ...
    protected void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective)
    {
        ...
        DrawOptions body = woodAxeMinion.initDraw().light(light).rotate(moveAngle, 14, 14).pos(drawX, drawY);
        topList.add((tm) -> {
            body.draw();
        });
    }
