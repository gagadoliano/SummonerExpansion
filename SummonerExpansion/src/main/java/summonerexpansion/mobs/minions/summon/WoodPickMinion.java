...
import static summonerexpansion.codes.registries.RegistryMinionTextures.woodPickMinion;

public class WoodPickMinion extends MagicToolBase
{
    private int toolHit;

    public WoodPickMinion()
    {
        super();
        color = new Color(226, 147, 0);
        searchRange = 400;
        setSpeed(40.0F);
    }
    ...
    protected void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective)
    {
        ...
        DrawOptions body = woodPickMinion.initDraw().light(light).rotate(moveAngle, 14, 14).pos(drawX, drawY);
        topList.add((tm) -> {
            body.draw();
        });
    }
