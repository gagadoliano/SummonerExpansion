...
import static summonerexpansion.codes.registries.RegistryMinionTextures.bookRunicShieldMinion;
...
public class BookRunicShieldMinion extends FlyingAttackingFollowingMob
{
    ...
    public void handleCollisionHit(Mob target, GameDamage damage, int knockback)
    {
        ...
        if (owner != null && target != null)
        {
            ...
            {
                buff = new ActiveBuff(BuffRegistry.getBuff("runicshieldbuff"), target, 61F, this);
            }
            else
            {
                buff = new ActiveBuff(BuffRegistry.getBuff("runicshieldbuff"), target, 31F, this);
            }
    ...
    protected void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective)
    {
        ...
        DrawOptions body = bookRunicShieldMinion.initDraw().sprite(0, 0, 32).light(light).pos(drawX, drawY);
        topList.add((tm) -> {
            body.draw();
        });
