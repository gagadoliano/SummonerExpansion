...
import static summonerexpansion.codes.registries.RegistryBuffs.WeaponBuffs.HONEYBEEBUFF;
import static summonerexpansion.codes.registries.RegistryMinionTextures.beeQueenMinion;

public class BeeQueenMinion extends SummonFlyingBase
{
    public int BeeHits;
    ...
    {
        ActiveBuff buff = new ActiveBuff(HONEYBEEBUFF, target, 6000, this);
    ...
    public void spawnDeathParticles(float knockbackX, float knockbackY)
    {
        for (int i = 0; i < 4; ++i)
        {
            getLevel().entityManager.addParticle(new FleshParticle(getLevel(), beeQueenMinion, i, 8, 32, x, y, 20.0F, knockbackX, knockbackY), Particle.GType.IMPORTANT_COSMETIC);
        }
    }
    ...
    public void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective)
    {
        ...
        final DrawOptions options = beeQueenMinion.initDraw().sprite(sprite.x, sprite.y, 64).addMaskShader(swimMask).light(light).pos(drawX, drawY);
