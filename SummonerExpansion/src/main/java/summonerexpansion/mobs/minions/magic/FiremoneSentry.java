...
import static summonerexpansion.codes.registries.RegistryMinionTextures.firemoneSentry;
...
public class FiremoneSentry extends SentryBase implements OEVicinityBuff
{
    ...
    public FiremoneSentry()
    {
        ...
        selectBox = new Rectangle(0, 0, 34, 54);
    }
    ...
    public void spawnDeathParticles(float knockbackX, float knockbackY)
    {
        for(int i = 0; i < 4; ++i)
        {
            this.getLevel().entityManager.addParticle(new FleshParticle(this.getLevel(), firemoneSentry, i, 8, 32, this.x, this.y, 20.0F, knockbackX, knockbackY), Particle.GType.IMPORTANT_COSMETIC);
        }
    }
    ...
    public void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective)
    {
        ...
        final TextureDrawOptionsEnd drawOptions = firemoneSentry.initDraw().sprite(sprite.x, sprite.y, 64).light(light).pos(drawX, drawY);
