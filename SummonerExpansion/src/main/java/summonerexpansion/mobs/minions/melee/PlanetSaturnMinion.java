...
import static summonerexpansion.codes.registries.RegistryMinionTextures.planetSaturnMinion;

public class PlanetSaturnMinion extends FlyingAttackingFollowingMob
{
    public static MaxHealthGetter MAX_HEALTH = new MaxHealthGetter(5, 10, 20, 30, 40);

    public PlanetSaturnMinion()
    {
        super(10);
    ...
    protected void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective)
    {
        ...
        DrawOptions body = planetSaturnMinion.initDraw().sprite(0, 0, 48).light(light).pos(drawX, drawY);
        topList.add((tm) -> {
            body.draw();
        });
