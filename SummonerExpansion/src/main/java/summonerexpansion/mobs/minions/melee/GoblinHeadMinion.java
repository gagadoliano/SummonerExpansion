...
import static summonerexpansion.codes.registries.RegistryMinionTextures.goblinHeadMinion;

public class GoblinHeadMinion extends AttackingFollowingMob
{
    public int lifeTime = 0;

    public GoblinHeadMinion()
    {
        super(10);
        setSpeed(35.0F);
        setFriction(3.0F);
    ...
    public void spawnDeathParticles(float knockbackX, float knockbackY)
    {
        for(int i = 0; i < 6; ++i)
        {
            this.getLevel().entityManager.addParticle(new FleshParticle(this.getLevel(), goblinHeadMinion, i, 8, 32, this.x, this.y, 20.0F, knockbackX, knockbackY), Particle.GType.IMPORTANT_COSMETIC);
        }
    }
    ...
    public void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective)
    {
        ...
        final DrawOptions options = goblinHeadMinion.initDraw().sprite(sprite.x, sprite.y, 64).addMaskShader(swimMask).light(light).pos(drawX, drawY);
