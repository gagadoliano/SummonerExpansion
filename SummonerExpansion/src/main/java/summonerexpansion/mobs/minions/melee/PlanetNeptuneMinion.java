...
import static summonerexpansion.codes.registries.RegistryMinionTextures.planetNeptuneMinion;

public class PlanetNeptuneMinion extends FlyingAttackingFollowingMob
{
    public static MaxHealthGetter MAX_HEALTH = new MaxHealthGetter(5, 10, 20, 30, 40);

    public PlanetNeptuneMinion()
    {
        super(10);
    ...
    protected void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective)
    {
        ...
        DrawOptions body = planetNeptuneMinion.initDraw().sprite(0, 0, 32).light(light).pos(drawX, drawY);
        topList.add((tm) -> {
            body.draw();
        });
