...
import static summonerexpansion.codes.registries.RegistryMinionTextures.fallenDragonBodyMinion;

public class FallenDragonBodyMinion extends FollowingWormMobBody<FallenDragonHeadMinion, FallenDragonBodyMinion>
{
    ...
    public void spawnDeathParticles(float knockbackX, float knockbackY)
    {
        if (this.isVisible())
        {
            for(int i = 0; i < 3; ++i)
            {
                this.getLevel().entityManager.addParticle(new FleshParticle(this.getLevel(), fallenDragonBodyMinion, 2, GameRandom.globalRandom.nextInt(6), 32, this.x, this.y, 20.0F, knockbackX, knockbackY), Particle.GType.IMPORTANT_COSMETIC);
            }
        }
    }
    ...
    protected void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective)
    {
        ...
        if (this.isVisible())
        {
            ...
            if (this.next != null)
            {
                ...
                MobDrawable drawOptions = WormMobHead.getAngledDrawable(new GameSprite(fallenDragonBodyMinion, 0, this.spriteY, 64), null, light, (int)this.height, angle, drawX, drawY, 96);
