package summonerexpansion.mobs.minions.worm;
...
import static summonerexpansion.codes.registries.RegistryMinionTextures.sandWormBodyMinion;
...
public class SandWormBodyMinion extends FollowingWormMobBody<SandWormHeadMinion, SandWormBodyMinion>
{
    ...
    public void spawnDeathParticles(float knockbackX, float knockbackY)
    {
        if (this.isVisible())
        {
            for(int i = 0; i < 3; ++i)
            {
                this.getLevel().entityManager.addParticle(new FleshParticle(this.getLevel(), sandWormBodyMinion, 2, GameRandom.globalRandom.nextInt(6), 32, this.x, this.y, 20.0F, knockbackX, knockbackY), Particle.GType.IMPORTANT_COSMETIC);
            }
        }
    }
