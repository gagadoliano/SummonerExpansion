...
import static summonerexpansion.codes.registries.RegistryMinionTextures.bookMushroomSentry;
...
public class BookMushroomSentry extends SentryBase implements OEVicinityBuff
{
    ...
    public void spawnDeathParticles(float knockbackX, float knockbackY)
    {
        for(int i = 0; i < 4; ++i)
        {
            this.getLevel().entityManager.addParticle(new FleshParticle(this.getLevel(), bookMushroomSentry, i, 8, 32, this.x, this.y, 20.0F, knockbackX, knockbackY), Particle.GType.IMPORTANT_COSMETIC);
        }
    }
    ...
    public void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective)
    {
        ...
        final TextureDrawOptionsEnd drawOptions = bookMushroomSentry.initDraw().sprite(sprite.x, sprite.y, 64).light(light).pos(drawX, drawY);
