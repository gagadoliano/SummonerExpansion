...
import static summonerexpansion.codes.registries.RegistryMinionTextures.leafShotHeatSentry;

public class LeafShotHeatSentry extends SentryBase
{
    ...
    {
        super.init();
        this.ai = new BehaviourTreeAI<>(this, new StationaryPlayerShooterAI<LeafShotHeatSentry>(600)
        {
            public void shootTarget(LeafShotHeatSentry mob, Mob target)
            {
            ...
            public Stream<Mob> streamTargets(LeafShotHeatSentry mob, int shootDistance)
            {
    ...
    public void spawnDeathParticles(float knockbackX, float knockbackY)
    {
        for(int i = 0; i < 4; ++i)
        {
            this.getLevel().entityManager.addParticle(new FleshParticle(this.getLevel(), leafShotHeatSentry, 10, i, 32, this.x, this.y, 20.0F, knockbackX, knockbackY), Particle.GType.IMPORTANT_COSMETIC);
        }
    }
    ...
    public void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective)
    {
        ...
        final DrawOptions body = leafShotHeatSentry.initDraw().sprite(spriteX, spriteY, 64).mirror(mirror, false).light(light).pos(drawX, drawY);
