...
import static summonerexpansion.codes.registries.RegistryMinionTextures.leafShotSentry;

public class LeafShotSentry extends SentryBase
{
    public LeafShotSentry()
    {
        super(3000F, 1000F);
        setFriction(2.0F);
    ...
    {
        super.init();
        this.ai = new BehaviourTreeAI<>(this, new StationaryPlayerShooterAI<LeafShotSentry>(600)
        {
            public void shootTarget(LeafShotSentry mob, Mob target)
            {
                float projVel = getAttackOwner().buffManager.getModifier(BuffModifiers.PROJECTILE_VELOCITY);

                Projectile projectile = ProjectileRegistry.getProjectile("leafballproj", mob.getLevel(), mob.x, mob.y, target.x, target.y, (120.0F * projVel), 640, summonDamage, mob);
            ...
            public Stream<Mob> streamTargets(LeafShotSentry mob, int shootDistance)
            {
    ...
    public void spawnDeathParticles(float knockbackX, float knockbackY)
    {
        for(int i = 0; i < 4; ++i)
        {
            this.getLevel().entityManager.addParticle(new FleshParticle(this.getLevel(), leafShotSentry, 10, i, 32, this.x, this.y, 20.0F, knockbackX, knockbackY), Particle.GType.IMPORTANT_COSMETIC);
        }
    }
    ...
    public void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective)
    {
        ...
        final DrawOptions body = leafShotSentry.initDraw().sprite(spriteX, spriteY, 64).mirror(mirror, false).light(light).pos(drawX, drawY);
