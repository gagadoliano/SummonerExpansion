...
import static summonerexpansion.codes.registries.RegistryMobTextures.horrorSpirit;
...
public class HorrorSpiritMob extends FlyingHostileMob
{
    ...
    public void spawnDeathParticles(float knockbackX, float knockbackY)
    {
        for(int i = 0; i < 4; ++i)
        {
            ...
            getLevel().entityManager.addParticle(new TopFleshParticle(getLevel(), horrorSpirit, 4 + sprite % 2, sprite / 2, 32, x, y, 20.0F, knockbackX, knockbackY), Particle.GType.IMPORTANT_COSMETIC);
        }
    }
    ...
    protected void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective)
    {
        ...
        DrawOptions body = horrorSpirit.initDraw().sprite(0, anim, 64).mirror(moveX < 0.0F, false).alpha(0.7F).light(light).pos(drawX, drawY);
        int minLight = 100;
        DrawOptions eyes = horrorSpirit.initDraw().sprite(1, anim, 64).mirror(moveX < 0.0F, false).alpha(0.7F).light(light.minLevelCopy((float)minLight)).pos(drawX, drawY);
