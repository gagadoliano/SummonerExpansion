package summonerexpansion.projectiles.armorset;
...
import static summonerexpansion.codes.registries.RegistryArmors.registerArmorSets.CLOUDWATERWEAK;
...
public class CloudWaterProj extends Projectile
{
    ...
    public void doHitLogic(Mob mob, LevelObjectHit object, float x, float y)
    {
        ...
        if (isServer())
        {
            if (mob != null)
            {
                ActiveBuff ab = new ActiveBuff(CLOUDWATERWEAK, mob, 60.0F, getOwner());
                mob.addBuff(ab, true);
            }
        }
    }
