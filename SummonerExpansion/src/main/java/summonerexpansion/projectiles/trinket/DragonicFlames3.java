...
import summonerexpansion.codes.events.DragonicBurningGroundEvent3;

import java.awt.*;
import java.awt.geom.Point2D;
import java.util.List;

public class DragonicFlames3 extends Projectile
{
    ...
    public DragonicFlames3() {
    }

    public DragonicFlames3(Level level, float x, float y, float targetX, float targetY, int distance, GameDamage damage, Mob owner, float upgradeTier)
    {
        this.setLevel(level);
        this.x = x;
        this.y = y;
        ...
        if (this.isServer())
        {
            while(this.distBuffer > (double)32.0F)
            {
                ...
                if (random.getChance(0.5F))
                {
                    ...
                    DragonicBurningGroundEvent3 event = new DragonicBurningGroundEvent3(this.getOwner(), eventX, eventY, random, this.getDamage(), this.firstGroundEventUniqueID, this.upgradeTier);
