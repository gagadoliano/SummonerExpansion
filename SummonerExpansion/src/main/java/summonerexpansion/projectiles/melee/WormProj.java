...
import summonerexpansion.mobs.minions.melee.ClawAncestorKnightMinion;

import java.awt.*;
import java.util.List;

import static summonerexpansion.codes.registries.RegistryMinionTextures.*;
...
public class WormProj extends FollowingProjectile
{
    ...
    public void doHitLogic(Mob mob, LevelObjectHit object, float x, float y)
    {
        ...
        if (mob != null && isServer())
        {
            ...
            {
                case 0:
                    mobFish = (AttackingFollowingMob)MobRegistry.getMob("fishcarpminion", getLevel());
                    break;

                case 1:
                    mobFish = (AttackingFollowingMob)MobRegistry.getMob("fishherringminion", getLevel());
                    break;

                case 2:
                    mobFish = (AttackingFollowingMob)MobRegistry.getMob("fishmackerelminion", getLevel());
                    break;

                case 3:
                    mobFish = (AttackingFollowingMob)MobRegistry.getMob("fishsalmonminion", getLevel());
                    break;

                case 4:
                    mobFish = (AttackingFollowingMob)MobRegistry.getMob("fishtroutminion", getLevel());
                    break;
            }

            if (GameRandom.globalRandom.nextInt(100) <= 10)
            {
                attackerMob.serverFollowersManager.addFollower("summonedfishminion", mobFish, FollowPosition.FLYING_CIRCLE, "summonedmob", 1.0F, (p) -> 3, null, false);
