...
import static summonerexpansion.codes.registries.RegistryTechs.SUMMONBOOKCRAFT;
...
public class SummoningBookshelf extends CraftingStationObject
{
    ...
    {
        super.loadTextures();
        texture = GameTexture.fromFile("objects/crafting/summoningbookshelf");
    }
    ...
    public void addDrawables(java.util.List<LevelSortedDrawable> list, OrderableDrawables tileList, Level level, int tileX, int tileY, TickManager tickManager, GameCamera camera, PlayerMob perspective)
    {
        ...
        final TextureDrawOptions options = texture.initDraw().sprite(rotation % 4, 0, 32, texture.getHeight()).addObjectDamageOverlay(this, level, tileX, tileY).alpha(alpha).light(light).pos(drawX, drawY - texture.getHeight() + 64);
    ...
    public void drawPreview(Level level, int tileX, int tileY, int rotation, float alpha, PlayerMob player, GameCamera camera)
    {
        int drawX = camera.getTileDrawX(tileX);
        int drawY = camera.getTileDrawY(tileY);
        texture.initDraw().sprite(rotation % 4, 0, 32, texture.getHeight()).addObjectDamageOverlay(this, level, tileX, tileY).alpha(alpha).draw(drawX, drawY - texture.getHeight() + 64);
    }
