...
public class EmpoweredNewRuby  extends TerrainSplatterTile
{
    private final GameRandom drawRandom;

    public EmpoweredNewRuby()
    {
        super(false, "empowerednewruby");
        mapColor = new Color(169, 37, 33);
        canBeMined = true;
    ...
    public void tick(Mob mob, Level level, int x, int y)
    {
        if (mob.isPlayer)
        {
            mob.buffManager.addBuff(new ActiveBuff(BuffRegistry.getBuff("rubyfloorbuff"), mob, 0.5F, mob), true);
        }
    }
    ...
    public ListGameTooltips getItemTooltips(InventoryItem item, PlayerMob perspective)
    {
        ...
        tooltips.add(Localization.translate("itemtooltip", "empoweredrubytip"));
        return tooltips;
    }
}