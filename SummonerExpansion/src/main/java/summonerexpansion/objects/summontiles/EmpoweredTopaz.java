...
public class EmpoweredTopaz  extends TerrainSplatterTile
{
    private final GameRandom drawRandom;

    public EmpoweredTopaz()
    {
        super(false, "empoweredtopaz");
        mapColor = new Color(212, 149, 73);
        canBeMined = true;
    ...
    public void tick(Mob mob, Level level, int x, int y)
    {
        if (mob.isPlayer)
        {
            mob.buffManager.addBuff(new ActiveBuff(BuffRegistry.getBuff("topazfloorbuff"), mob, 0.5F, mob), true);
        }
    }
    ...
    public ListGameTooltips getItemTooltips(InventoryItem item, PlayerMob perspective)
    {
        ...
        tooltips.add(Localization.translate("itemtooltip", "empoweredtopaztip"));
        return tooltips;
    }
}