...
public class OceanMonkShrineWorldPreset extends WorldPreset
{
    ...
    public void addToRegion(GameRandom random, LevelPresetsRegion presetsRegion, final BiomeGeneratorStack generatorStack, PerformanceTimerManager performanceTimer)
    {
        ...
        {
            final Point tile = findRandomBiomePresetTile(random, presetsRegion, generatorStack, BiomeRegistry.DESERT, 20, size, new String[] { "loot" }, (tileX, tileY) -> (generatorStack.isSurfaceOcean(tileX, tileY) && generatorStack.getLazyBiomeID(tileX, tileY) == BiomeRegistry.DESERT.getID()));
            if (tile != null)
            {
                presetsRegion.addPreset(this, tile.x, tile.y, size, new String[]{"loot"},
                        (random1, level, timer) ->
                        {
                            ...
                            OceanMonkShrinePreset preset = new OceanMonkShrinePreset(random1);
