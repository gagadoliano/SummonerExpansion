package summonerexpansion.biomes.presets.Snow;
...
public class ChristmasShrineWorldPreset extends WorldPreset
{
    ...
    {
        int total = getTotalBiomePoints(random, presetsRegion, BiomeRegistry.SNOW, 0.001F);
        for (int i = 0; i < total; i++)
        {
            final Point tile = findRandomBiomePresetTile(random, presetsRegion, generatorStack, BiomeRegistry.SNOW, 20, size, new String[] { "loot", "villages" }, (tileX, tileY) -> (!generatorStack.isSurfaceExpensiveWater(tileX, tileY) && generatorStack.getLazyBiomeID(tileX, tileY) == BiomeRegistry.SNOW.getID()));
            if (tile != null)
            {
                presetsRegion.addPreset(this, tile.x, tile.y, size, new String[]{"loot", "villages"},
                        (random1, level, timer) ->
                        {
                            ...
                            ChristmasShrinePreset preset = new ChristmasShrinePreset(random1);
