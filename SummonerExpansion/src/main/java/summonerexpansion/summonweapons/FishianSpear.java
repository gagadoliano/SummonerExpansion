...
{
    public IntUpgradeValue maxFishians = new IntUpgradeValue(2, 0.0F).setUpgradedValue(1, 4).setUpgradedValue(5, 6);
    public int fishianHit;
    ...
    public FishianSpear()
    {
        ...
        attackDamage.setBaseValue(30.0F).setUpgradedValue(1.0F, 45.0F);
    ...
    public InventoryItem onAttack(Level level, int x, int y, ItemAttackerMob attackerMob, int attackHeight, InventoryItem item, ItemAttackSlot slot, int animAttack, int seed, GNDItemMap mapContent)
    {
        ...
        if (animAttack == 0)
        {
            if (attackerMob.isServer() && fishianHit >= 100)
            {
                ...
                attackerMob.serverFollowersManager.addFollower("fishianminion", mob, FollowPosition.WALK_CLOSE, "summonedmob", 1.0F, maxFishians.getValue(getUpgradeTier(item)), null, false);
    ...
    public ListGameTooltips getPreEnchantmentTooltips(InventoryItem item, PlayerMob perspective, GameBlackboard blackboard)
    {
        ...
        tooltips.add(Localization.translate("itemtooltip", "chargetip", "amount", fishianHit));
