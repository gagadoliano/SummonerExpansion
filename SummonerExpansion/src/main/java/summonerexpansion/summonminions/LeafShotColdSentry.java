...
public class LeafShotColdSentry extends AttackingFollowingMob
{
    ...
    {
        super.init();
        ai = new BehaviourTreeAI<>(this, new PlayerFollowerChaserAI<LeafShotColdSentry>(600, 600, false, false, 90000, 64)
        {
            public boolean attackTarget(LeafShotColdSentry mob, Mob target)
            {
                if (mob.canAttack())
                {
                    ...
                    Projectile projectile = ProjectileRegistry.getProjectile("leafcoldballproj", mob.getLevel(), mob.x, mob.y, target.x, target.y, 120F, 640, summonDamage, mob);
