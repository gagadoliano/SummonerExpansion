...
public class JellyfishMinion extends FlyingAttackingFollowingMob
{
    ...
    public JellyfishMinion()
    {
        ...
        playBoltSoundAbility = this.registerAbility(new EmptyMobAbility()
        {
            protected void run()
            {
                if (JellyfishMinion.this.isClient()) 
                {
    ...
    {
        super.init();
        ai = new BehaviourTreeAI<>(this, new PlayerFollowerChaserAI<JellyfishMinion>(500, 300, false, false, 800, 70)
        {
            public boolean attackTarget(JellyfishMinion mob, Mob target) 
            {
                if (JellyfishMinion.this.canAttack() && !JellyfishMinion.this.isOnGenericCooldown("attackCooldown"))
                {
                    ...
                    JellyfishMinion.this.startGenericCooldown("attackCooldown", JellyfishMinion.this.attackCooldown);
                    return true;
                }
                else
                {
