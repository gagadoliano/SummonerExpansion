...
public class MummyMagicMinion extends AttackingFollowingMob
{
    protected int deathTime = 4000;
    protected int lifeTime = 0;

    public MummyMagicMinion()
    {
        super(10);
        setSpeed(40.0F);
        setFriction(3.0F);
    ...
    {
        super.init();
        ai = new BehaviourTreeAI<>(this, new PlayerFollowerChaserAI<MummyMagicMinion>(600, 500, false, false, 640, 64)
        {
            public boolean attackTarget(MummyMagicMinion mob, Mob target)
            {
                if (mob.canAttack())
                {
                    mob.attack(target.getX(), target.getY(), false);
                    Projectile projectile = ProjectileRegistry.getProjectile("quartzbolt", mob.getLevel(), mob.x, mob.y, target.x, target.y, 80.0F, 800, summonDamage, mob);
