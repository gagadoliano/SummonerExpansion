...
public class EnchantedBabyZombieArcherMinion extends AttackingFollowingMob
{
    ...
    {
        super.init();
        this.ai = new BehaviourTreeAI<>(this, new PlayerFollowerChaserAI<EnchantedBabyZombieArcherMinion>(600, 320, false, false, 640, 64)
        {
            public boolean attackTarget(EnchantedBabyZombieArcherMinion mob, Mob target)
            {
                if (mob.canAttack())
                {
                    ...
                    Projectile projectile = ProjectileRegistry.getProjectile("zombiearrow", mob.getLevel(), mob.x, mob.y, target.x, target.y, 80.0F, 640, EnchantedBabyZombieArcherMinion.this.summonDamage, mob);
