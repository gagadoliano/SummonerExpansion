...
public class MagicPickaxeMinion extends FlyingAttackingFollowingMob
{
    ...
    {
        super.clientTick();
        getLevel().entityManager.addParticle(x + (float)(GameRandom.globalRandom.nextGaussian() * 4.0), y + (float)(GameRandom.globalRandom.nextGaussian() * 4.0), Particle.GType.IMPORTANT_COSMETIC).movesConstant(this.dx / 10.0F, this.dy / 10.0F).color(new Color(226, 147, 0));
    }
    ...
    public void spawnDeathParticles(float knockbackX, float knockbackY)
    {
        for(int i = 0; i < 30; ++i)
        {
            getLevel().entityManager.addParticle(x, y, Particle.GType.COSMETIC).sprite(GameResources.bubbleParticle.sprite(0, 0, 12)).movesConstantAngle((float) GameRandom.globalRandom.nextInt(360), (float)GameRandom.globalRandom.getIntBetween(5, 20)).color(new Color(226, 147, 0));
        }
    }
