...
public class AgedChampionMinion extends AttackingFollowingMob
{
    protected int lookSeed;
    protected HumanLook look;

    public AgedChampionMinion()
    {
        super(10);
        setSpeed(45.0F);
        setFriction(3.0F);
    ...
    public void init()
    {
        super.init();
        updateLook();
        ai = new BehaviourTreeAI(this, new PlayerFollowerCollisionChaserAI<AgedChampionMinion>(500, summonDamage, 60, 1000, 900, 80)
        {
            public boolean attackTarget(AgedChampionMinion mob, Mob target)
            {
                if (AgedChampionMinion.this.canAttack())
                {
                    ...
                    AgedChampionMinion.this.getLevel().entityManager.addLevelEvent(new ToolItemMobAbilityEvent(AgedChampionMinion.this, GameRandom.globalRandom.nextInt(), attackItem, mob.getX(), mob.getY(), AgedChampionMinion.this.attackAnimTime, AgedChampionMinion.this.attackAnimTime));
                    return true;
                }
                else
                {
