...
import summonerexpansion.projectiles.mount.MountZombieArrowProj;
...
public class ZombieArcherSummonMount extends BaseTransformMount implements MountAbility
{
    ...
    public void runMountAbility(PlayerMob player, Packet content)
    {
        if (isServer())
        {
            ...
            player.getLevel().entityManager.projectiles.add(new MountZombieArrowProj(x, y, camera.getMouseLevelPosX(), camera.getMouseLevelPosY(), damage, player));
            abilityCooldown = 10;
        }
    }
