...
public class GiantPumpkinBuff extends TrinketBuff
{
    static GameDamage damage = new GameDamage(DamageTypeRegistry.SUMMON, 50);

    public GiantPumpkinBuff() {}
    ...
    public void serverTick(ActiveBuff buff)
    {
        if (buff.owner.isItemAttacker)
        {
            ...
            float count = attackerMob.serverFollowersManager.getFollowerCount("pumpkinminion");
            if (count <= 0.0F)
            {
                Level level = buff.owner.getLevel();
                AttackingFollowingMob mob = (AttackingFollowingMob) MobRegistry.getMob("pumpkinminion", level);
                attackerMob.serverFollowersManager.addFollower("pumpkinminion", mob, FollowPosition.WALK_CLOSE, "summonedpumpkinminionbuff", 1.0F, 1, null, false);
    ...
    public void onRemoved(ActiveBuff buff)
    {
        ...
        if (buff.owner.isServer() && buffManager.hasBuff("summonedpumpkinminionbuff"))
        {
            buffManager.removeBuff("summonedpumpkinminionbuff", true);
        }
    }
    ...
    public ListGameTooltips getTrinketTooltip(TrinketItem trinketItem, InventoryItem item, PlayerMob perspective)
    {
        ...
        tooltips.add(Localization.translate("itemtooltip", "pumpkinminiontip"));
        return tooltips;
    }
}
