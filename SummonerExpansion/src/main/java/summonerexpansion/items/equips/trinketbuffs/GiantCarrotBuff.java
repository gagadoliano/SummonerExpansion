...
public class GiantCarrotBuff extends TrinketBuff
{
    static GameDamage damage = new GameDamage(DamageTypeRegistry.SUMMON, 20);

    public GiantCarrotBuff() {}

    public void init(ActiveBuff buff, BuffEventSubscriber eventSubscriber)
    {
        buff.setModifier(BuffModifiers.MINING_RANGE, 0.20F);
    }
    ...
    public void serverTick(ActiveBuff buff)
    {
        ...
        if (buff.owner.isItemAttacker)
        {
            ...
            float count = attackerMob.serverFollowersManager.getFollowerCount("carrotminion");
            if (count <= 0.0F)
            {
                Level level = buff.owner.getLevel();
                AttackingFollowingMob mob = (AttackingFollowingMob) MobRegistry.getMob("carrotminion", level);
                attackerMob.serverFollowersManager.addFollower("carrotminion", mob, FollowPosition.WALK_CLOSE, "summonedcarrotminionbuff", 1.0F, 1, null, false);
    ...
    public void onRemoved(ActiveBuff buff)
    {
        ...
        {
            buffManager.removeBuff("summonedcarrotminionbuff", true);
        }
    }
    ...
    public ListGameTooltips getTrinketTooltip(TrinketItem trinketItem, InventoryItem item, PlayerMob perspective)
    {
        ...
        tooltips.add(Localization.translate("itemtooltip", "carrotminiontip"));
        return tooltips;
    }
}