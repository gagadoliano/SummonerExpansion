...
import summonerexpansion.mobs.summonminions.LeafShotHeatSentry;

import java.awt.geom.Point2D;

public class LeafShotHeatPack extends SettlerIgnoredThrowToolItem
{
    public LeafShotHeatPack(Item.Rarity rarityTier)
    {
        rarity = rarityTier;
        damageType = DamageTypeRegistry.SUMMON;
        attackAnimTime.setBaseValue(300);
        attackDamage.setBaseValue(18.0F);
        stackSize = 250;
    ...
    public InventoryItem onAttack(Level level, int x, int y, ItemAttackerMob attackerMob, int attackHeight, InventoryItem item, ItemAttackSlot slot, int animAttack, int seed, GNDItemMap mapContent)
    {
        ...
        {
            LeafShotHeatSentry mob = new LeafShotHeatSentry();
            Point2D.Float dir = GameMath.normalize((float)x - attackerMob.x, (float)y - attackerMob.y + (float)attackHeight);
            attackerMob.serverFollowersManager.addFollower("summonedplantsentry", mob, FollowPosition.WALK_CLOSE, "summonedmob", 1.0F, 6, null, false);
    ...
    public ListGameTooltips getPreEnchantmentTooltips(InventoryItem item, PlayerMob perspective, GameBlackboard blackboard)
    {
        ...
        tooltips.add(Localization.translate("itemtooltip", "leafshotheatpacktip"));
        return tooltips;
    }
}