...
import summonerexpansion.mobs.minions.melee.MimeMinion;

import java.awt.geom.Point2D;

public class MimeSword extends BaseSummonSwordWeapon
{
    public IntUpgradeValue minionGroupSize = (new IntUpgradeValue()).setBaseValue(1);

    public MimeSword(int enchantCost, Item.Rarity rarityTier)
    {
        super(enchantCost, rarityTier);
        attackDamage.setBaseValue(20).setUpgradedValue(1, 200);
    ...
    public void hitMob(InventoryItem item, ToolItemMobAbilityEvent event, Level level, Mob target, Mob attacker)
    {
        ...
        {
            ActiveBuff ab = new ActiveBuff(BuffRegistry.getBuff("mimestack"), attacker, 31F, attacker);
    ...
    public InventoryItem onAttack(Level level, int x, int y, ItemAttackerMob attackerMob, int attackHeight, InventoryItem item, ItemAttackSlot slot, int animAttack, int seed, GNDItemMap mapContent)
    {
        ...
        if (animAttack == 0 && attackerMob.isServer() && attackerMob.buffManager.getStacks(BuffRegistry.getBuff("mimestack")) >= 30)
        {
            MimeMinion mob = new MimeMinion();
            Point2D.Float dir = GameMath.normalize((float)x - attackerMob.x, (float)y - attackerMob.y + (float)attackHeight);
            attackerMob.serverFollowersManager.addFollower("summonedmimeminion", mob, FollowPosition.WALK_CLOSE, "summonedmob", 1.0F, minionGroupSize.getValue(getUpgradeTier(item)), null, false);
            ...
            attackerMob.buffManager.removeBuff("mimestack", true);
        }
        return item;
    }
    ...
    public ListGameTooltips getPreEnchantmentTooltips(InventoryItem item, PlayerMob perspective, GameBlackboard blackboard)
    {
        ...
        tooltips.add(Localization.translate("itemtooltip", "mimeswordtip"));
