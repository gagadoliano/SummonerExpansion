...
import summonerexpansion.codes.registries.RegistryArmors;
import summonerexpansion.projectiles.armorset.CopperSetProj;
...
public class CopperMinerSetBonus extends SetBonusBuff implements BuffAbility
{
    ...
    {
        return abilityDamage.getValue(buff.getUpgradeTier()) > 0 && player.buffManager.getStacks(BuffRegistry.SUMMONED_MOB) > 0 && !buff.owner.buffManager.hasBuff(RegistryArmors.registerArmorSets.COPPERCOOLDOWN) && !buff.owner.buffManager.hasBuff(RegistryArmors.registerArmorSets.COPPERSET_CONSECUTIVE);
    }
    ...
    public void runAbility(PlayerMob player, ActiveBuff buff, Packet content)
    {
        ...
        player.buffManager.addBuff(new ActiveBuff(RegistryArmors.registerArmorSets.COPPERCOOLDOWN, player, abilityCooldown, null), false);
    ...
    protected boolean shotNextSummon(ActiveBuff ab)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                ActiveBuff cooldownBuff = ab.owner.buffManager.getBuff(RegistryArmors.registerArmorSets.COPPERCOOLDOWN.getID());
                if (cooldownBuff != null && cooldownBuff.getDurationLeft() < stackBuff.getDuration())
                {
                    ab.owner.buffManager.addBuff(new ActiveBuff(RegistryArmors.registerArmorSets.COPPERCOOLDOWN, ab.owner, stackBuff.getDuration(), null), true);
                }
                return true;
            }
        }
