...
import static summonerexpansion.codes.registries.RegistryMinionTextures.setHorrorBabyMinion;
...
public class SetHorrorBabyMinion extends FlyingAttackingFollowingMob
{
    ...
    protected void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective)
    {
        ...
        DrawOptions body = setHorrorBabyMinion.initDraw().sprite(0, 0, 32).light(light).rotate(this.moveAngle, 16, 16).pos(drawX, drawY);
        int minLight = 100;
        DrawOptions eyes = setHorrorBabyMinion.initDraw().sprite(1, 0, 32).light(light.minLevelCopy((float)minLight)).rotate(this.moveAngle, 16, 16).pos(drawX, drawY);
        topList.add((tm) -> {
            body.draw();
            eyes.draw();
        });
