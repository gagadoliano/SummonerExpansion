...
import static summonerexpansion.codes.registries.RegistryMinionTextures.setSpiderBrideMinion;
...
public class SetSpiderBrideMinion extends AttackingFollowingMob
{
    ...
    public void spawnDeathParticles(float knockbackX, float knockbackY)
    {
        for(int i = 0; i < 10; ++i)
        {
            this.getLevel().entityManager.addParticle(new FleshParticle(this.getLevel(), setSpiderBrideMinion, 14 + i / 5, i % 5, 48, this.x, this.y, 20.0F, knockbackX, knockbackY), Particle.GType.IMPORTANT_COSMETIC);
        }
    }
    ...
    public void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective)
    {
        ...
        final DrawOptions body = setSpiderBrideMinion.initDraw().sprite(sprite.x, sprite.y, 96).addMaskShader(swimMask).light(light).pos(drawX, drawY);
