...
import static summonerexpansion.codes.registries.RegistryMinionTextures.setTitaniumMagicMinion;
...
public class SetTitaniumMagicMinion extends FlyingAttackingFollowingMob
{
    ...
    public void spawnDeathParticles(float knockbackX, float knockbackY)
    {
        for(int i = 0; i < 4; ++i)
        {
            getLevel().entityManager.addParticle(new FleshParticle(getLevel(), setTitaniumMagicMinion, i, 2, 32, x, y, 20.0F, knockbackX, knockbackY), Particle.GType.IMPORTANT_COSMETIC);
        }
    }
    ...
    protected void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective)
    {
        ...
        DrawOptions options = setTitaniumMagicMinion.initDraw().sprite(0, 0, 54).light(light).rotate(currentAngle + 45.0F - 90.0F, 26, 26).pos(drawX, drawY);
