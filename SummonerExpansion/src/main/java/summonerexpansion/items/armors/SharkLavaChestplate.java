...
public class SharkLavaChestplate extends ChestArmorItem
{
    ...
    {
        return new ArmorModifiers(new ModifierValue<>(BuffModifiers.SUMMONS_SPEED, summonSpeed.getValue(getUpgradeTier(item))), new ModifierValue<>(BuffModifiers.RESILIENCE_GAIN, resilienceGain.getValue(getUpgradeTier(item))), new ModifierValue<>(BuffModifiers.SWIM_SPEED, speedSwim.getValue(getUpgradeTier(item))));
    }
