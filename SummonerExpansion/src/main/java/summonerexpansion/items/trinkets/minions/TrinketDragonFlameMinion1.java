...
public class TrinketDragonFlameMinion1 extends FlyingAttackingFollowingMob
{
    public int lifeTime = 0;
    public TrinketDragonFlameMinion1()
    {
        super(10);
        setSpeed(50.0F);
        setFriction(2.0F);
    ...
    {
        super.init();
        ai = new BehaviourTreeAI<>(this, new PlayerFollowerChaserAI<TrinketDragonFlameMinion1>(400, 300, false, false, 800, 64)
        {
            public boolean attackTarget(TrinketDragonFlameMinion1 mob, Mob target)
            {
                if (mob.canAttack())
                {
                    mob.attack(target.getX(), target.getY(), false);
                    Projectile projectile = ProjectileRegistry.getProjectile("pyromancy", mob.getLevel(), mob.x, mob.y, target.x, target.y, 60F, 640, summonDamage, mob);
    ...
    public void spawnDeathParticles(float knockbackX, float knockbackY)
    {
        for(int i = 0; i < 4; ++i)
        {
            getLevel().entityManager.addParticle(new FleshParticle(getLevel(), MobRegistry.Textures.dragonWhelp, i, 8, 32, x, y, 20.0F, knockbackX, knockbackY), Particle.GType.IMPORTANT_COSMETIC);
        }
    }
    ...
    public void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective)
    {
        ...
        final DrawOptions options = MobRegistry.Textures.dragonWhelp.initDraw().sprite(aboveLiquid ? anim : sprite.x, sprite.y, 64).addMaskShader(swimMask).startGlowOptions(level, getID()).light(light).applyEnemyTracker(this, perspective).pos(drawX, drawY - height);
