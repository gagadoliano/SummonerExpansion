...
public class CaveSwampMushroomShrineWorldPreset extends WorldPreset
{
    protected Dimension size = new Dimension(6, 7);
    ...
    public void addToRegion(GameRandom random, LevelPresetsRegion presetsRegion, final BiomeGeneratorStack generatorStack, PerformanceTimerManager performanceTimer)
    {
        ...
        {
            final Point tile = findRandomBiomePresetTile(random, presetsRegion, generatorStack, BiomeRegistry.SWAMP, 20, size, new String[] { "loot" }, (tileX, tileY) -> (!generatorStack.isSurfaceExpensiveWater(tileX, tileY) && generatorStack.getLazyBiomeID(tileX, tileY) == BiomeRegistry.SWAMP.getID()));
            if (tile != null)
            {
                presetsRegion.addPreset(this, tile.x, tile.y, size, new String[]{"loot"},
                        (random1, level, timer) ->
                        {
                            ...
                            CaveSwampMushroomShrinePreset preset = new CaveSwampMushroomShrinePreset();
