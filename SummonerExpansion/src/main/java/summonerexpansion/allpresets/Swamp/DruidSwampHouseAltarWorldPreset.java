...
public class DruidSwampHouseAltarWorldPreset extends WorldPreset
{
    protected Dimension size = new Dimension(10, 11);
    ...
    {
        int total = getTotalBiomePoints(random, presetsRegion, BiomeRegistry.SWAMP, 0.001F);
        for (int i = 0; i < total; i++)
        {
            final Point tile = findRandomBiomePresetTile(random, presetsRegion, generatorStack, BiomeRegistry.SWAMP, 20, size, new String[] { "loot", "villages" }, (tileX, tileY) -> (!generatorStack.isSurfaceExpensiveWater(tileX, tileY) && generatorStack.getLazyBiomeID(tileX, tileY) == BiomeRegistry.SWAMP.getID()));
            if (tile != null)
            {
                presetsRegion.addPreset(this, tile.x, tile.y, size, new String[]{"loot", "villages"},
                        (random1, level, timer) ->
                        {
                            ...
                            DruidSwampHouseAltarPreset preset = new DruidSwampHouseAltarPreset(random1);
