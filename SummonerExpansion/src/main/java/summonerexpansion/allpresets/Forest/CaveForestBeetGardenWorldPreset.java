...
public class CaveForestBeetGardenWorldPreset extends WorldPreset
{
    protected Dimension size = new Dimension(13, 12);
    ...
    public void addToRegion(GameRandom random, LevelPresetsRegion presetsRegion, final BiomeGeneratorStack generatorStack, PerformanceTimerManager performanceTimer)
    {
        ...
        {
            final Point tile = findRandomBiomePresetTile(random, presetsRegion, generatorStack, BiomeRegistry.FOREST, 20, size, new String[] {"loot"}, (tileX, tileY) -> (!generatorStack.isSurfaceExpensiveWater(tileX, tileY) && generatorStack.getLazyBiomeID(tileX, tileY) == BiomeRegistry.FOREST.getID()));
            if (tile != null)
            {
                presetsRegion.addPreset(this, tile.x, tile.y, size, new String[]{"loot"},
                        (random1, level, timer) ->
                        {
                            ...
                            CaveForestBeetGardenPreset preset = new CaveForestBeetGardenPreset();
