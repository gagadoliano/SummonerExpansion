...
import static summonerexpansion.summonothers.SummonerTextures.cavelingminecart_mask;

public class CavelingMinecartMount extends MountFollowingMob
{
    ...
    protected GameMessage getSummonLocalization() {
        return MobRegistry.getLocalization("cavelingminecart");
    }
    ...
    protected void tickCollisionMovement(float delta, Mob rider)
    {
        ...
        if (object instanceof MinecartTrackObject && !(object instanceof TrapTrackObject))
        {
            ...
            if (pos != null)
            {
                ...
                if (minecartSpeed > 0.0F)
                {
                    MinecartLinePos resultPos = pos.progressLines(minecartDir, minecartSpeed * delta / 250.0F, (Consumer)null);
                    x = resultPos.x;
                    y = resultPos.y;
