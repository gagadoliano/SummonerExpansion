...
import summonerexpansion.codes.particles.DragonicBurningGroundParticle2;

import java.awt.*;

import static summonerexpansion.codes.registries.RegistryDebuffs.WeaponDebuffs.DRAGON_FROSTDOT;

public class DragonicBurningGroundEvent2 extends GroundEffectEvent
{
    private GameDamage damage = new GameDamage(0.0F);
    protected MobHitCooldowns hitCooldowns = new MobHitCooldowns(1000);
    protected int tickCounter;
    protected int hitCounter;
    private DragonicBurningGroundParticle2 particle;
    protected int hitCooldownMasterUniqueID;
    protected float upgradeTier;

    public DragonicBurningGroundEvent2() {
    }

    public DragonicBurningGroundEvent2(Mob owner, int x, int y, GameRandom uniqueIDRandom, GameDamage damage, int hitCooldownMasterUniqueID, float upgradeTier)
    {
    ...
    public void init()
    {
        ...
        if (this.isClient())
        {
            this.level.entityManager.addParticle(this.particle = new DragonicBurningGroundParticle2(this.level, (float)this.x, (float)this.y, 2000L), true, Particle.GType.CRITICAL);
    ...
    public void serverHit(Mob target, boolean clientSubmitted)
    {
        ...
        {
            ActiveBuff buff = new ActiveBuff(DRAGON_FROSTDOT, target, 10.0F, this.owner);
