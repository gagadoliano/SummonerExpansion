...
import static summonerexpansion.codes.registries.RegistryParticlesTextures.groundFireParticles3;

public class DragonicBurningGroundParticle3 extends Particle
{
    private int particleBuffer = 0;
    private final int randomAnimOffset;

    public DragonicBurningGroundParticle3(Level level, float x, float y, long lifeTime)
    {
    ...
    public void addDrawables(List<LevelSortedDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, OrderableDrawables overlayList, Level level, TickManager tickManager, GameCamera camera, PlayerMob perspective)
    {
        ...
        DrawOptions options = groundFireParticles3.initDraw().sprite(anim, 0, 32).pos(drawX, drawY).alpha(alpha);
