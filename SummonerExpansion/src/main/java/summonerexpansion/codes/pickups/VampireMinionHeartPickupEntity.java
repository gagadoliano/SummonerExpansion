...
public class VampireMinionHeartPickupEntity extends PickupEntity 
{
    protected ParticleTypeSwitcher particleTypeSwitcher;
    protected int tickCounter;
    public float healPercent;

    public VampireMinionHeartPickupEntity() 
    {
    ...
    public VampireMinionHeartPickupEntity(Level level, float x, float y, float dx, float dy) 
    {
    ...
    public void init()
    {
        ...
        if (isClient())
        {
            ...
            for(int i = 0; i < 10; ++i)
            {
                getLevel().entityManager.addParticle(x + random.floatGaussian() * 16.0F, y + random.floatGaussian() * 12.0F, Particle.GType.IMPORTANT_COSMETIC).sizeFades(24, 12).sprite(GameResources.bubbleParticle.sprite(0, 0, 12)).color(new Color(223, 12, 12)).movesFrictionAngle((float)random.getIntBetween(0, 360), 50.0F, 0.5F).lifeTime(1000);
            }
        }
    }
    ...
    public void clientTick()
    {
        ...
        {
            super.clientTick();
            getLevel().entityManager.addParticle(x + (float)(GameRandom.globalRandom.nextGaussian() * (double)8.0F), y + (float)(GameRandom.globalRandom.nextGaussian() * (double)6.0F), particleTypeSwitcher.next()).color(new Color(129, 33, 29)).movesFriction(0.0F, -30.0F, 0.8F).givesLight(353.0F, 0.8F).ignoreLight(true).lifeTime(500);
        }
    }
    ...
    public void onPickup(ServerClient client)
    {
        ...
        client.playerMob.buffManager.addBuff(new ActiveBuff(BuffRegistry.BLOODPLATE_COWL_ACTIVE, client.playerMob, 10F, null), true);
    ...
    public void addDrawables(List<LevelSortedDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, OrderableDrawables overlayList, Level level, TickManager tickManager, GameCamera camera, PlayerMob perspective) 
    {
        ...
        final DrawOptions pickupOptions = GameResources.vampiricHeartPickup.initDraw().sprite(spriteIndex, 0, 32).light(light).size(32 - spriteIndex * 2, 32 - spriteIndex * 2).pos(drawX - 16 + spriteIndex, drawY - 16 + spriteIndex);
